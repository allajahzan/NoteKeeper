...
interface propsType {
    editBar: boolean,
    editNoteFunction: (note: Type) => void,
    handleEditBar: () => void,
    noteToEdit: Type | null
}

function EditNote({ editBar, editNoteFunction, handleEditBar, noteToEdit }: propsType) {
    ...
    useEffect(() => {
        if (editBar) {
            ...
            // set input title and content to edit
            TitleInputChange(noteToEdit?.title as string)
            ContentInputChange(noteToEdit?.content as string)
            setAction(() => handleEditBar)
    ...
    let handleSubmit = (event: React.FormEvent) => {
        ...
        editNoteFunction({ title: titleInput, content: contentInput, _id: noteToEdit?._id })
        TitleInputChange('')
        ContentInputChange('')
        handleEditBar()
    }
    ...
    return (
        <>
            <div onClick={action} style={style2} className="flex justify-start w-[100vw] h-[100vh] fixed top-0 right-0 transition-all duration-300 bg-opacity-5 overflow-hidden">
